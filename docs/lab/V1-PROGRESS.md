...
| 2 — decay arithmetic | expected RED: missing-module exit 1; exponent mismatch 1 failure; context/operand regressions 7 failures and 6 errors; precision-boundary clamp 1 failure | bundled Python 3.12; focused 11 tests and full 28-test validation passed, 0 failed; checksums and `git diff --check` passed | approved | context/operand and exact reducing-only clamp findings fixed; re-review pending | review pending |
